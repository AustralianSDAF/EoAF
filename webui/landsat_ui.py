"""
Gradio WebUI for Landsat-8/9 data search, download, processing and visualisation.
Author: Calvin Pang
Date: 2024-03-14
"""
...
raw_dir = Path(config["LANDSAT_RAW"], session_id)
processed_dir = Path(config["LANDSAT_PROCESSED"], session_id)
visualisation_dir = Path(config["LANDSAT_VISUALISED"], session_id)
...
def landsat_search(
    ...
    """
    Perform a search for Landsat satellite data based on the specified parameters from Gradio components.
...
def landsat_download(
    ...
    """
    Downloads Landsat satellite data based on the specified parameters from Gradio components.
    ...
    Args:
        ...
        collection (str): The collection of Landsat data to be downloaded.
...
def landsat_process(
    ...
    """
    Process Landsat satellite data based on the specified parameters from Gradio components.
    ...
    Returns:
        None
    """
    # Retrieving all downloaded Landsat-8/9 directories
...
def landsat_merge(selected_products: list):
    """
    Merge processed Landsat satellite data based on the specified parameters from Gradio components.
...
if __name__ == "__main__":
    with gr.Blocks() as demo:
        ...
        gr.Markdown("#Eyes on Australian Forests - Landsat-8 & 9")
        gr.Textbox(label="Session ID", value=session_id, interactive=False)

        with gr.Tab("Search & Download"):
            gr.Markdown("## Search for Landsat-8 and/or 9 Products")
            gr.Markdown(
                "Please enter your Landsat-8 or 9 search parameters below. You may either define a bounding box or optionally upload a shapefile."
            )
            with gr.Row():
            ...
            with gr.Row():
                collection = gr.Radio(
                    label="Select a Landsat-8 or 9 collection",
                    choices=["ga_ls8c_ard_3", "ga_ls9c_ard_3"],
                    value="ga_ls8c_ard_3",
                    interactive=True,
                )
            ...
            gr.Markdown("## Download Landsat-8 and/or 9 Products")
            gr.Markdown(
        ...
        with gr.Tab("Process"):
            ...
            with gr.Column():
                gr.Markdown(
                    "Please select your raw Landsat-8 or 9 products to process. You may select all products by checking the box below."
                )
            ...
            with gr.Accordion(label="Merge Processed Products (Optional)", open=True):
                gr.Markdown(
                    "You may merge multiple processed Landsat-8 or 9 products into a single product. Please select the processed products to merge."
                )
                selected_processed_products = gr.FileExplorer(
                    label="Select products(s) to merge", root_dir=processed_dir, every=5
                )
        ...
        with gr.Tab("Visualisation"):
            ...
            gr.Markdown(
                "Please select your processed Landsat-8 or 9 products to visualise. It will be saved as a HTML file for you to download and open via a web browser."
            )
            with gr.Row():
            ...
            with gr.Row():
                resample_selection = gr.Checkbox(
                    label="Apply a downscaling factor to reduce the spatial resolution of product for rapid display?", value=False
                )
